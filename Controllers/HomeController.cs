       ...
       // String PathServer = @"D:\vs projects\ServerContainer\";
        ...
        public ActionResult Index()
        {
            if (System.IO.File.Exists(PathServer + @"App_Data\DBInfo\" + User.Identity.Name + @"\InfoDB.txt")){
                ...
                string[] file = System.IO.File.ReadAllLines(PathServer + @"App_Data\DBInfo\" + User.Identity.Name + @"\InfoDB.txt");
                return View(file);
            }
            return View();
        }
        ...
        public ActionResult DB(string type="",string status="")
        {
            ...
            model.Status = db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "mysql").Status;
            modelList.Add(model);
            model = new FileModel();
            ...
            model.Status = db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "postgresql").Status;
            modelList.Add(model);
            model = new FileModel();
            model.Name = "NPM";
            model.Reference = "<a href=\"http://172.19.2.63:4873/\" target=\"_blank\">http://172.19.2.63:4873/</a>-npm ";
            model.Status = db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "npm").Status;
            modelList.Add(model);
            return View(modelList);
            ...
            using (p = new Process())
            {
                try
                {
                    ...
                    if (name == "MySQL")
                    {
                        ...
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "mysql").Status="Контейнеры запущены";
                    } else if(name == "NPM")
                    {
                        psi.WorkingDirectory = PathServer + @"App_Data\Dockerfiles\npm";
                        //Session["StatusMySQL"] = "Запущено";
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "npm").Status = "Контейнеры запущены";
                    }
                    else
                    {
                        psi.WorkingDirectory = PathServer + @"App_Data\Dockerfiles\postgresql";
                        // Session["StatusPostgreSQL"] = "Запущено";
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "postgresql").Status = "Контейнеры запущены";
                    }
                    db.SaveChanges();
            ...
            using (p = new Process())
            {
                try
                {
                    ...
                    if (name == "MySQL")
                    {
                        psi.WorkingDirectory = PathServer + @"App_Data\Dockerfiles\mysql";
                        //Session["StatusMySQL"] = "Не запущено";
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "mysql").Status = "Контейнеры не запущены";
                    }
                    else if (name == "NPM")
                    {
                        psi.WorkingDirectory = PathServer + @"App_Data\Dockerfiles\npm";
                        //Session["StatusMySQL"] = "Не запущено";
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "npm").Status = "Контейнеры не запущены";
                    }
                    else
                    {
                        psi.WorkingDirectory = PathServer + @"App_Data\Dockerfiles\postgresql";
                        // Session["StatusPostgreSQL"] = "Не запущено";
                        db.ApplicationPort.FirstOrDefault(ap => ap.NameApplication == "postgresql").Status = "Контейнеры не запущены";
                    }
                    db.SaveChanges();
                    psi.FileName = "cmd.exe";
        ...
        public ActionResult LogSite(string name = "")
        {
            ...
            process.StartInfo.UseShellExecute = false; // Флажок в false для перенаправления вывода
            ...
            process.StartInfo.RedirectStandardError = true; // Перенаправляем стандартный поток ошибок процесса
            process.Start();
        ...
        private async void myProcess_Exited(object sender, System.EventArgs e,string name,string subject)
        {
           ...
           // string s = "docker run--name " + name + "_cont - p " + port + ":" + port2 + "--network networkDB " + name;
            psi.WorkingDirectory = @"C:\Windows\System32";
            string s= @"/c docker run --name " + name + "_cont -p " + port + ":" + port2 + " --network networkDB " + name;
            psi.FileName = "cmd.exe";
            psi.Arguments = @"/c docker run --name "+name+"_cont -p " + port + ":" + port2 + " --network db_networkDB " + name;
            psi.CreateNoWindow = true;
            var p = Process.Start(psi);
            await WriteLogAsync(@"/c docker run --name " + name + "_cont -p " + port + ":" + port2 + " --network db_networkDB " + name, PathServer + @"App_Data\LogSite.txt");
        ...
        public ActionResult Upload(HttpPostedFileBase upload)
        {
            if (upload != null)
            {
                ...
                // получаем имя файла
                string fileName = System.IO.Path.GetFileName(Guid.NewGuid()+"_"+upload.FileName.ToLower());
        ...
        public async Task<ActionResult> DownloadDumpPostgreSQL(string name)
        {
            ...
            process.StartInfo.Arguments = "/c docker exec -u root -it postgres-server pg_dump -U postgres "+name+"db  > "+name+".sql";
            process.Start();
            await Task.WhenAny(eventHandled.Task, Task.Delay(9990000));
        ...
        public async Task<ActionResult> DownloadDumpMySQL(string name)
        {
            var process = new Process();
            process.StartInfo.FileName = @"C:\Windows\System32\cmd.exe";
            process.StartInfo.WorkingDirectory = PathServer + @"App_Data\Projects\" + name;
            ...
            process.EnableRaisingEvents = true;
            process.Exited += new EventHandler(myProcess_ExitedDB);
            process.StartInfo.Arguments = "/c docker exec -it mysql-server mysqldump -u root -pHello_227 " + name+ "db  > " + name + ".sql";
            process.Start();
            await Task.WhenAny(eventHandled.Task, Task.Delay(9990000));
